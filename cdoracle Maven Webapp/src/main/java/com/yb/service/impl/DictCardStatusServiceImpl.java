...
public class DictCardStatusServiceImpl implements DictCardStatusService{
	...
	@Autowired
	private DictCardStatusDao dictCardStatusDao;
	...
	public void queryAll() {
		// TODO Auto-generated method stub
				...
				int count=0;
				try {
					File file = new File("dictCardStatusCDSHELL.txt");
					if(!file.exists()){
					...
					List<DictCardStatus> list = dictCardStatusDao.queryAll(id);
					if(list.size()!=0&&list!=null){//取出来的有数据，才会调用
						ArrayList<DictCardStatus> arrayList = new ArrayList<DictCardStatus>();
						for (DictCardStatus dictCardStatus : list) {
							arrayList.add(dictCardStatus);//添加的一条数据
							count++;
							if(count%30==0){
								String jsonString = JSON.toJSONString(arrayList);
								HttpEntity httpEntity = new StringEntity(jsonString,"UTF-8");
						        String asString = Request.Post("http://localhost:8989/cdserver/dictCardStatus/insert")
						...
						Integer maxId = dictCardStatusDao.queryMaxId();
