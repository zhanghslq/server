...
public class DictCarduserStatusServiceImpl implements DictCarduserStatusService{
	...
	@Autowired
	private DictCarduserStatusDao dictCarduserStatusDao;
	...
	public void queryAll() {
		// TODO Auto-generated method stub
				...
				try {
					...
					List<DictCarduserStatus> list = dictCarduserStatusDao.queryAll(id);
					if(list.size()!=0&&list!=null){//取出来的有数据，才会调用
						ArrayList<DictCarduserStatus> arrayList = new ArrayList<DictCarduserStatus>();
						for (DictCarduserStatus dictCarduserStatus: list) {
							arrayList.add(dictCarduserStatus);//添加的一条数据
							count++;
							if(count%30==0){
								String jsonString = JSON.toJSONString(arrayList);
								HttpEntity httpEntity = new StringEntity(jsonString,"UTF-8");
						        String asString = Request.Post("http://localhost:8989/cdserver/dictCarduserStatus/insert")
						...
						Integer maxId = dictCarduserStatusDao.queryMaxId();
