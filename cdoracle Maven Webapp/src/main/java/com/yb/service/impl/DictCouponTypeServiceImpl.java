...
public class DictCouponTypeServiceImpl implements DictCouponTypeService{
	...
	@Autowired
	private DictCouponTypeDao dictCouponTypeDao;
	...
	public void queryAll() {
		// TODO Auto-generated method stub
				...
				try {
					...
					List<DictCouponType> list = dictCouponTypeDao.queryAll(id);
					if(list.size()!=0&&list!=null){//取出来的有数据，才会调用
						ArrayList<DictCouponType> arrayList = new ArrayList<DictCouponType>();
						for (DictCouponType dictCouponType : list) {
							arrayList.add(dictCouponType);//添加的一条数据
							count++;
							if(count%30==0){
								String jsonString = JSON.toJSONString(arrayList);
								HttpEntity httpEntity = new StringEntity(jsonString,"UTF-8");
						        String asString = Request.Post("http://localhost:8989/cdserver/dictCouponType/insert")
						...
						Integer maxId = dictCouponTypeDao.queryMaxId();
