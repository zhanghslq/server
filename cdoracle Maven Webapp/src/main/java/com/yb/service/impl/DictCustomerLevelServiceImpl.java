...
public class DictCustomerLevelServiceImpl implements DictCustomerLevelService{
	...
	@Autowired
	private DictCustomerLevelDao dictCustomerLevelDao;
	...
	public void queryAll() {
		...
		// TODO Auto-generated method stub
				...
				try {
					...
					List<DictCustomerLevel> list = dictCustomerLevelDao.queryAll(id);
					if(list.size()!=0&&list!=null){//取出来的有数据，才会调用
						ArrayList<DictCustomerLevel> arrayList = new ArrayList<DictCustomerLevel>();
						for (DictCustomerLevel dictCustomerLevel : list) {
							arrayList.add(dictCustomerLevel);//添加的一条数据
							count++;
							if(count%30==0){
								String jsonString = JSON.toJSONString(arrayList);
								HttpEntity httpEntity = new StringEntity(jsonString,"UTF-8");
						        String asString = Request.Post("http://localhost:8989/cdserver/dictCustomerLevel/insert")
						...
						Integer maxId = dictCustomerLevelDao.queryMaxId();
