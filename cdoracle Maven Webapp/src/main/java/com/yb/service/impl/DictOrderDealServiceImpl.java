...
public class DictOrderDealServiceImpl implements DictOrderDealService{
	...
	@Autowired
	private DictOrderDealDao dictOrderDealDao;
	...
	public void queryAll() {
		// TODO Auto-generated method stub
				...
				try {
					...
					List<DictOrderDeal> list = dictOrderDealDao.queryAll(id);
					if(list.size()!=0&&list!=null){//取出来的有数据，才会调用
						ArrayList<DictOrderDeal> arrayList = new ArrayList<DictOrderDeal>();
						for (DictOrderDeal dictOrderDeal : list) {
							arrayList.add(dictOrderDeal);//添加的一条数据
							count++;
							if(count%30==0){
								String jsonString = JSON.toJSONString(arrayList);
								HttpEntity httpEntity = new StringEntity(jsonString,"UTF-8");
						        String asString = Request.Post("http://localhost:8989/cdserver/dictOrderDeal/insert")
						...
						Integer maxId = dictOrderDealDao.queryMaxId();
