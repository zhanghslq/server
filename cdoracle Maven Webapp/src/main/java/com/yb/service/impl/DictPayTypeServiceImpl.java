...
public class DictPayTypeServiceImpl implements DictPayTypeService{
	...
	@Autowired
	private DictPayTypeDao dictPayTypeDao;
	...
	public void queryAll() {
		// TODO Auto-generated method stub
				...
				try {
					...
					List<DictPayType> list = dictPayTypeDao.queryAll(id);
					if(list.size()!=0&&list!=null){//取出来的有数据，才会调用
						ArrayList<DictPayType> arrayList = new ArrayList<DictPayType>();
						for (DictPayType dictPayType : list) {
							arrayList.add(dictPayType);//添加的一条数据
							count++;
							if(count%30==0){
								String jsonString = JSON.toJSONString(arrayList);
								HttpEntity httpEntity = new StringEntity(jsonString,"UTF-8");
						        String asString = Request.Post("http://localhost:8989/cdserver/dictPayType/insert")
						...
						Integer maxId = dictPayTypeDao.queryMaxId();
