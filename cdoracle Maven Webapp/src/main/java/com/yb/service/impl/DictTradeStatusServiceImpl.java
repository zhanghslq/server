...
public class DictTradeStatusServiceImpl implements DictTradeStatusService{
	...
	@Autowired
	private DictTradeStatusDao dictTradeStatusDao;
	...
	public void queryAll() {
		// TODO Auto-generated method stub
				...
				try {
					...
					List<DictTradeStatus> list = dictTradeStatusDao.queryAll(id);
					if(list.size()!=0&&list!=null){//取出来的有数据，才会调用
						ArrayList<DictTradeStatus> arrayList = new ArrayList<DictTradeStatus>();
						for (DictTradeStatus dictTradeStatus : list) {
							arrayList.add(dictTradeStatus);//添加的一条数据
							count++;
							if(count%30==0){
								String jsonString = JSON.toJSONString(arrayList);
								HttpEntity httpEntity = new StringEntity(jsonString,"UTF-8");
						        String asString = Request.Post("http://localhost:8989/cdserver/dictTradeStatus/insert")
						...
						Integer maxId = dictTradeStatusDao.queryMaxId();
