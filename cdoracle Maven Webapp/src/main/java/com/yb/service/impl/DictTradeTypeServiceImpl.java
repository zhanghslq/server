...
public class DictTradeTypeServiceImpl implements DictTradeTypeService{
	...
	@Autowired
	private DictTradeTypeDao dictTradeTypeDao;
	...
	public void queryAll() {
		// TODO Auto-generated method stub
				...
				int count=0;
				try {
					File file = new File("dictTradeTypeCDSHELL.txt");
					if(!file.exists()){
					...
					List<DictTradeType> list = dictTradeTypeDao.queryAll(id);
					if(list.size()!=0&&list!=null){//取出来的有数据，才会调用
						ArrayList<DictTradeType> arrayList = new ArrayList<DictTradeType>();
						for (DictTradeType dictTradeType : list) {
							arrayList.add(dictTradeType);//添加的一条数据
							count++;
							if(count%30==0){
								String jsonString = JSON.toJSONString(arrayList);
								HttpEntity httpEntity = new StringEntity(jsonString,"UTF-8");
						        String asString = Request.Post("http://localhost:8989/cdserver/dictTradeType/insert")
						...
						Integer maxId = dictTradeTypeDao.queryMaxId();
