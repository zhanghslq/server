...
public class DictVirtualGoodsStatusServiceImpl implements DictVirtualGoodsStatusService{
	...
	@Autowired
	private DictVirtualGoodsStatusDao dictVirtualGoodsStatusDao;
	...
	public void queryAll() {
		// TODO Auto-generated method stub
				...
				int count=0;
				try {
					File file = new File("dictVirtualGoodsStatusCDSHELL.txt");
					if(!file.exists()){
					...
					List<DictVirtualGoodsStatus> list = dictVirtualGoodsStatusDao.queryAll(id);
					if(list.size()!=0&&list!=null){//取出来的有数据，才会调用
						ArrayList<DictVirtualGoodsStatus> arrayList = new ArrayList<DictVirtualGoodsStatus>();
						for (DictVirtualGoodsStatus dictVirtualGoodsStatus : list) {
							arrayList.add(dictVirtualGoodsStatus);//添加的一条数据
							count++;
							if(count%30==0){
								String jsonString = JSON.toJSONString(arrayList);
								HttpEntity httpEntity = new StringEntity(jsonString,"UTF-8");
						        String asString = Request.Post("http://localhost:8989/cdserver/dictVirtualGoodsStatus/insert")
						...
						Integer maxId = dictVirtualGoodsStatusDao.queryMaxId();
