...
import com.yb.dao.DictVirtualGoodsTypeDao;
...
public class DictVirtualGoodsTypeServiceImpl implements DictVirtualGoodsTypeService{
	...
	@Autowired
	private DictVirtualGoodsTypeDao dictVirtualGoodsTypeDao;
	...
	public void queryAll() {
		// TODO Auto-generated method stub
				...
				try {
					...
					List<DictVirtualGoodsType> list = dictVirtualGoodsTypeDao.queryAll(id);
					if(list.size()!=0&&list!=null){//取出来的有数据，才会调用
						ArrayList<DictVirtualGoodsType> arrayList = new ArrayList<DictVirtualGoodsType>();
						for (DictVirtualGoodsType dictVirtualGoodsType : list) {
							arrayList.add(dictVirtualGoodsType);//添加的一条数据
							count++;
							if(count%30==0){
								String jsonString = JSON.toJSONString(arrayList);
								HttpEntity httpEntity = new StringEntity(jsonString,"UTF-8");
						        String asString = Request.Post("http://localhost:8989/cdserver/dictVirtualGoodsType/insert")
						...
						Integer maxId = dictVirtualGoodsTypeDao.queryMaxId();
