	...
	public void queryAll() {
		...
		// TODO Auto-generated method stub
				...
				try {
					...
					List<TblCardtype> list = tblCardTypeDao.queryAll(id);
					if(list.size()!=0&&list!=null){//取出来的有数据，才会调用
						ArrayList<TblCardtype> arrayList = new ArrayList<TblCardtype>();
						for (TblCardtype tblCardType : list) {
							arrayList.add(tblCardType);//添加的一条数据
							count++;
							if(count%30==0){
								String jsonString = JSON.toJSONString(arrayList);
								HttpEntity httpEntity = new StringEntity(jsonString,"UTF-8");
						        String asString = Request.Post("http://localhost:8989/cdserver/tblCardtype/insert")
						...
						Integer maxId = tblCardTypeDao.queryMaxId();
