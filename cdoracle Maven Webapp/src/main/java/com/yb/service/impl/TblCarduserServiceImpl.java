...
public class TblCarduserServiceImpl implements TblCarduserService{
	...
	public void queryAll() {
		...
		int count=0;
		try {
			File file = new File("TblCarduserCDSHELL.txt");
			if(!file.exists()){
				writer = new PrintWriter(file);
				Long queryMinId = tblCarduserDao.queryMinId();
			...
			List<TblCarduser> list = tblCarduserDao.queryAll(id,id+100000);
			
			
			
			if(list.size()!=0){//取出来的有数据，才会调用
				ArrayList<TblCarduser> arrayList = new ArrayList<TblCarduser>();
				for (TblCarduser tblCarduser : list) {
					arrayList.add(tblCarduser);//添加的一条数据
					count++;
					if(count%40==0){
						String jsonString = JSON.toJSONString(arrayList);
						HttpEntity httpEntity = new StringEntity(jsonString,"UTF-8");
				        String asString = Request.Post("http://localhost:8989/cdserver/tblCarduser/insert")
				...
				list.clear();
				if(arrayList.size()!=0){//证明最后还有未提交的数据
					String jsonString = JSON.toJSONString(arrayList);
					HttpEntity httpEntity = new StringEntity(jsonString,"UTF-8");
			        String asString = Request.Post("http://localhost:8989/cdserver/tblCarduser/insert")
			                .body(httpEntity).setHeader("content-type", "application/json;charset=UTF-8")
				...
				pw=new PrintWriter(file);
				Long queryMinId = tblCarduserDao.queryMinId();
				pw.write(String.valueOf(queryMinId));//把最新的id最大的写入文件
				pw.flush();
			}
		} catch (Exception e) {
