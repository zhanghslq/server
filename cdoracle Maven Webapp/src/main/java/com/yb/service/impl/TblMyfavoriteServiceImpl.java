...
public class TblMyfavoriteServiceImpl implements TblMyfavoriteService {
	...
	public void queryAll() {
		...
		try {
			...
			List<TblMyfavorite> list = tblMyfavoriteDao.queryAll(id);
			if(list.size()!=0&&list!=null){//取出来的有数据，才会调用
				ArrayList<TblMyfavorite> arrayList = new ArrayList<TblMyfavorite>();
				for (TblMyfavorite tblMyfavorite : list) {
					arrayList.add(tblMyfavorite);//添加的一条数据
					count++;
					if(count%30==0){
						String jsonString = JSON.toJSONString(arrayList);
						HttpEntity httpEntity = new StringEntity(jsonString,"UTF-8");
				        String asString = Request.Post("http://localhost:8989/cdserver/tblMyfavorite/insert")
				...
				Long maxId = tblMyfavoriteDao.queryMaxId();
