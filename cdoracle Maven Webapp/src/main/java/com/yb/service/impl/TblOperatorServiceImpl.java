...
public class TblOperatorServiceImpl implements TblOperatorService{
	...
	public void queryAll() {
		...
		// TODO Auto-generated method stub
				...
				int count=0;
				try {
					File file = new File("TblOperatorCDSHELL.txt");
					if(!file.exists()){
					...
					List<TblOperator> list = tblOperatorDao.queryAll(id);
					if(list.size()!=0&&list!=null){//取出来的有数据，才会调用
						ArrayList<TblOperator> arrayList = new ArrayList<TblOperator>();
						for (TblOperator tblOperator : list) {
							arrayList.add(tblOperator);//添加的一条数据
							count++;
							if(count%30==0){
								String jsonString = JSON.toJSONString(arrayList);
								HttpEntity httpEntity = new StringEntity(jsonString,"UTF-8");
						        String asString = Request.Post("http://localhost:8989/cdserver/tblOperator/insert")
