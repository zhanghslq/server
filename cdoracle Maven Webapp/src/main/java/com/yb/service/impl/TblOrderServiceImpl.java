...
public class TblOrderServiceImpl implements TblOrderService{
	...
	public void queryAll() {
		...
		try {
			...
			List<TblOrder> list = tblOrderDao.queryAll(id);
			if(list.size()!=0&&list!=null){//取出来的有数据，才会调用
				ArrayList<TblOrder> arrayList = new ArrayList<TblOrder>();
				for (TblOrder tblOrder : list) {
					arrayList.add(tblOrder);//添加的一条数据
					count++;
					if(count%30==0){
						String jsonString = JSON.toJSONString(arrayList);
						HttpEntity httpEntity = new StringEntity(jsonString,"UTF-8");
				        String asString = Request.Post("http://localhost:8989/cdserver/tblOrder/insert")
				...
				Long maxId = tblOrderDao.queryMaxId();
