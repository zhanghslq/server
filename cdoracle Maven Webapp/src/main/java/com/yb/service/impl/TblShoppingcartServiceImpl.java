...
public class TblShoppingcartServiceImpl implements TblShoppingcartService{
	...
	public void queryAll() {
		...
		try {
			...
			List<TblShoppingcart> list = tblShoppingcartDao.queryAll(id);
			if(list.size()!=0&&list!=null){//取出来的有数据，才会调用
				ArrayList<TblShoppingcart> arrayList = new ArrayList<TblShoppingcart>();
				for (TblShoppingcart tblShoppingcart : list) {
					arrayList.add(tblShoppingcart);//添加的一条数据
					count++;
					if(count%30==0){
						String jsonString = JSON.toJSONString(arrayList);
						HttpEntity httpEntity = new StringEntity(jsonString,"UTF-8");
				        String asString = Request.Post("http://localhost:8989/cdserver/tblShoppingcart/insert")
