...
public class CouponOilOrderServiceImpl implements CouponOilOrderService{
	...
	public void queryAll() {
		...
		int count=0;
		try {
			File file = new File("CouponOilOrder.txt");
			if(!file.exists()){
			...
			List<CouponOilOrder> list = couponOilOrderDao.queryAll(id);
			if(list.size()!=0&&list!=null){//取出来的有数据，才会调用
				ArrayList<CouponOilOrder> arrayList = new ArrayList<CouponOilOrder>();
				for (CouponOilOrder couponOilOrder : list) {
					arrayList.add(couponOilOrder);//添加的一条数据
					count++;
					if(count%30==0){
						String jsonString = JSON.toJSONString(arrayList);
						HttpEntity httpEntity = new StringEntity(jsonString,"UTF-8");
				        String asString = Request.Post("http://localhost:8989/server/couponOilOrder/insert")
				...
				Long maxId = couponOilOrderDao.queryMaxId();
