...
public class CouponWechatServiceImpl implements CouponWechatService{
	...
	public void queryAll() {
		...
		int count=0;
		try {
			File file = new File("CouponWechat.txt");
			if(!file.exists()){
			...
			List<CouponWechat> list = couponWechatDao.queryAll(id);
			if(list.size()!=0&&list!=null){//取出来的有数据，才会调用
				ArrayList<CouponWechat> arrayList = new ArrayList<CouponWechat>();
				for (CouponWechat couponWechat : list) {
					arrayList.add(couponWechat);//添加的一条数据
					count++;
					if(count%30==0){
						String jsonString = JSON.toJSONString(arrayList);
						HttpEntity httpEntity = new StringEntity(jsonString,"UTF-8");
				        String asString = Request.Post("http://localhost:8989/server/couponWechat/insert")
				...
				Long maxId = couponWechatDao.queryMaxId();
